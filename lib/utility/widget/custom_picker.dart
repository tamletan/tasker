...
// }
//
// class MCupertinoDatePicker extends StatefulWidget {
//   MCupertinoDatePicker({
//     Key key,
//     this.mode = MDatePickerMode.dateAndTime,
...
//     assert(
//       mode != MDatePickerMode.dateAndTime ||
//           minimumDate == null ||
...
//     switch (mode) {
//       case MDatePickerMode.time:
//       case MDatePickerMode.dateAndTime:
//         return _CupertinoDatePickerDateTimeState();
//       case MDatePickerMode.date:
//         return _CupertinoDatePickerDateState();
//       case MDatePickerMode.month:
//         return _CupertinoDatePickerMonthState();
//         break;
//       case MDatePickerMode.year:
//         return _CupertinoDatePickerYearState();
//         break;
//     }
//
//     assert(false);
...
//
// class _CupertinoDatePickerDateTimeState extends State<MCupertinoDatePicker> {
...
//
//   @override
//   void didUpdateWidget(MCupertinoDatePicker oldWidget) {
...
//           MCupertinoDatePicker._getColumnWidth(
...
//     }
//
//     if (widget.mode == MDatePickerMode.dateAndTime) {
//       if (localizations.datePickerDateTimeOrder ==
//               DatePickerDateTimeOrder.time_dayPeriod_date ||
//           localizations.datePickerDateTimeOrder ==
...
// }
//
// class _CupertinoDatePickerDateState extends State<MCupertinoDatePicker> {
//   int textDirectionFactor;
//   CupertinoLocalizations localizations;
//
//   Alignment alignCenterLeft;
...
// }
//
// class _CupertinoDatePickerMonthState extends State<MCupertinoDatePicker> {
//   int textDirectionFactor;
//   CupertinoLocalizations localizations;
//
//   int selectedMonth;
...
//     final DateTime maxSelectedDate = DateTime(selectedYear, selectedMonth);
//
//     final bool minCheck = widget.minimumDate?.isBefore(maxSelectedDate) ?? true;
//     final bool maxCheck =
//         widget.maximumDate?.isBefore(minSelectedDate) ?? false;
...
//     final DateTime maxSelectDate = DateTime(selectedYear, selectedMonth);
//
//     final bool minCheck = widget.minimumDate?.isBefore(maxSelectDate) ?? true;
//     final bool maxCheck = widget.maximumDate?.isBefore(minSelectDate) ?? false;
//
...
//         columnWidths = <double>[screenWidth / 2, screenWidth / 2];
//         break;
//       case DatePickerDateOrder.ymd:
//       case DatePickerDateOrder.ydm:
...
// }
//
// class _CupertinoDatePickerYearState extends State<MCupertinoDatePicker> {
//   int textDirectionFactor;
//   CupertinoLocalizations localizations;
//
//   int selectedYear;
...
//     columnWidths = <double>[MediaQuery.of(context).size.width];
//
//     final List<Widget> pickers = <Widget>[];
//
//     pickers.add(LayoutId(
...
enum MCupertinoDatePickerMode {
  ...
  dateAndTime,
  // Custom mode for only month and year
  month,
  year,
}
...
class MCupertinoDatePicker extends StatefulWidget {
  ...
  MCupertinoDatePicker({
    Key key,
    this.mode = MCupertinoDatePickerMode.dateAndTime,
    @required this.onDateTimeChanged,
    DateTime initialDateTime,
    this.minimumDate,
    this.maximumDate,
    ...
    assert(this.initialDateTime != null);
    assert(
      mode != MCupertinoDatePickerMode.dateAndTime ||
          minimumDate == null ||
          !this.initialDateTime.isBefore(minimumDate),
      'initial date is before minimum date',
    );
    ...
    assert(
      mode != MCupertinoDatePickerMode.date ||
          (minimumYear >= 1 && this.initialDateTime.year >= minimumYear),
      'initial year is not greater than minimum year, or minimum year is not positive',
    );
    assert(
  ...
  final MCupertinoDatePickerMode mode;
  ...
  State<StatefulWidget> createState() {
    ...
    switch (mode) {
      ...
      case MCupertinoDatePickerMode.dateAndTime:
        return _CupertinoDatePickerDateTimeState();
...
class _CupertinoDatePickerDateTimeState extends State<MCupertinoDatePicker> {
  ...
  // The difference in days between the initial date and the currently
  // selected date.
  ...
  // because the meridiem picker can be scrolled **animatedly**
  // by the hour picker
  ...
  // (meridiem picker invariant) region of the hour picker.
  // The "physical" region
  ...
  void _onSelectedItemChange(int index) {
    ...
    final isDateInvalid = widget.minimumDate?.isAfter(selected) == true ||
        widget.maximumDate?.isBefore(selected) == true;
        ...
        child: CupertinoPicker(
          ...
          onSelectedItemChanged: (index) {
            ...
            if (!widget.use24hFormat && regionChanged) {
              // Scroll the meridiem column to adjust AM/PM.
              //
              // _onSelectedItemChanged will be called when the animation
              // finishes.
              //
              // Animation values obtained by comparing with iOS version.
              meridiemController.animateToItem(
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.mode == MCupertinoDatePickerMode.dateAndTime) {
      if (localizations.datePickerDateTimeOrder ==
              DatePickerDateTimeOrder.time_dayPeriod_date ||
          localizations.datePickerDateTimeOrder ==
              DatePickerDateTimeOrder.dayPeriod_time_date) {
