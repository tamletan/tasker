...
public class MyBroacastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if(intent.getAction().equals(Config.msg_broadcast_on_sv_destroy)){
            if(rod) startSV(context);
        }

        if(intent.getAction().equals(Config.msg_broadcast_set_rod_on)){
            rod = true;
        }

        if(intent.getAction().equals(Config.msg_broadcast_set_rod_off)){
            rod = false;
        }

        if(intent.getAction().equals(Config.msg_broadcast_set_rob_on)){
            rob = true;
        }

        if(intent.getAction().equals(Config.msg_broadcast_set_rob_off)){
            rob = false;
        }
    }
